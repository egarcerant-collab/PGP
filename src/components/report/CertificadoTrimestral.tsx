        ...
        content: [
          ...
          ...(showNotaAclaratoria ? [{
            table: {
              ...
              body: [[{
                stack: [
                  { text: 'NOTA ACLARATORIA — PAGOS ANTICIPADOS', bold: true, fontSize: 8.5, color: '#1e3a8a', margin: [0, 0, 0, 4] },
                  { text: 'Los valores registrados en el presente cuadro corresponden a valores brutos proyectados conforme al modelo de pago del contrato, antes de deducciones. El valor neto efectivamente girado difiere del valor bruto aquí consignado por efecto de las retenciones tributarias aplicables (retención en la fuente, ICA y demás tributos según corresponda). La conciliación de esta diferencia se realizará con base en el certificado de giros expedido por la Tesorería de DUSAKAWI EPSI.', fontSize: 7, color: '#1e3a8a', margin: [0, 0, 0, 5] },
                  { text: `El presente informe evalúa exclusivamente la ejecución técnico-financiera del contrato ${contratoNum} correspondiente a la vigencia ${new Date().getFullYear()}, período ${periodo.toLowerCase()}. Los saldos y carteras de vigencias anteriores no son objeto de esta evaluación; su depuración corresponde al proceso de liquidación del respectivo contrato y se formaliza mediante las actas de cierre de la vigencia a la que pertenecen. En consecuencia, el presente cuadro no constituye certificación de pagos efectuados, conciliación de cartera ni paz y salvo entre las partes.`, fontSize: 7, color: '#1e3a8a' },
                ],
                fillColor: '#dbeafe',
              }]],
            },
                        ...
                        <tbody>
                          {trim.infs.map((inf: any) => (
                            <tr key={inf.numero} className="border-t border-border/40 hover:bg-muted/20">
                              ...
                              {/* Acta Firmada — solo para períodos consolidados (múltiples meses, ej. FEB-MAR-ABR) */}
                              <td className="px-3 py-1.5 text-center">
                                {inf.actaFirmadaUrl ? (
